...
ax[2].plot(radar.index ,radar['z_elev'],marker='.',linestyle='',label='radar', color='orange')
...
for i,timeshift in enumerate(table.time_shift):
    selection = (radar['file_name'] == table.file_name[i]) & (radar['line_name'] == table.line_name[i])
    if table.broken[i] == 0:
        radar.loc[selection, 'index_shift'] = radar.index[selection] + timedelta(hours=timeshift) - timedelta(minutes=3.5)
    if table.broken[i] == 1:
        sub_selection_1 = (radar['file_name'] == table.file_name[i]) & (radar['line_name'] == table.line_name[i])&(radar.index > pd.to_datetime(table.broken_time[i]))
        radar['index_shift'][sub_selection_1] = radar.index[sub_selection_1] + timedelta(hours=table.time_shift_2[i]) - timedelta(minutes=3.5)
        sub_selection_2 = (radar['file_name'] == table.file_name[i]) & (radar['line_name'] == table.line_name[i])&(radar.index < pd.to_datetime(table.broken_time[i]))
...
ax[2].plot(gps_hh.index, gps_hh['ele'], marker='.',linestyle='',label='gps - hand held', color='grey')
ax[2].plot(radar.index_shift ,radar['z_elev'],marker='.',linestyle='',label='radar - corrected', color='orange')
...
file_names = radar['file_name'].unique()

for file_name in file_names:
    index = radar['file_name'] == file_name
    ...
    line_names = radar[index]['line_name'].unique()
    color = sns.color_palette("flare",n_colors = len(line_names))
    for j,line_name in enumerate(line_names):
        sub_index = (radar['file_name'] == file_name)&(radar['line_name'] == line_name)
...
ax.plot(radar.index_shift ,radar['z_elev'], marker='.',linestyle='',label='radar', color='cyan')
